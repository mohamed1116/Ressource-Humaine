...
_LOGO_PATH = _os.path.join(_os.path.dirname(__file__), '..', '..', '..', '..', '..', 'hrms-frontend', 'public', 'logo-fpt.png')
try:
    _LOGO_B64 = 'data:image/png;base64,' + _b64.b64encode(open(_os.path.abspath(_LOGO_PATH), 'rb').read()).decode()
except Exception:
    _LOGO_B64 = ''
...
<div style="text-align:center; margin-bottom:15px;">
    <img src="{_LOGO_B64}" alt="Logo FPT" style="height:100px; width:auto;">
</div>
"""
...
TEMPLATES = [
    # 1
    {
        ...
        'vars': [
            auto('employee_name', 'Nom et Prenom'),
            auto('position', 'Grade'),
            auto('numero_somme', 'N de Somme'),
            auto('cin', 'CIN'),
            auto('hire_date', 'Date de recrutement'),
            auto('date_today', 'Date du jour'),
            auto('civilite', 'Civilite (Monsieur/Madame)'),
        ],
        'content': build(
            T('Attestation de Travail'),
            P("Le Doyen de La Facult&eacute; Polydisciplinaire de Taroudant atteste que {{civilite}} :"),
            '<p style="text-align:left; font-size:12pt; line-height:2.5; margin-left:40px;">'
            '<u><strong>Nom et Pr&eacute;nom</strong></u> : &nbsp;<strong>{{employee_name}}</strong><br>'
            '<u><strong>Grade</strong></u> : &nbsp;{{position}}<br>'
            '<u><strong>SOM</strong></u> &nbsp;&nbsp;: &nbsp;{{numero_somme}}<br>'
            '<u><strong>CIN</strong></u> &nbsp;&nbsp;&nbsp;&nbsp;: &nbsp;{{cin}}'
            '</p>',
            P("Est en fonction &agrave; la Facult&eacute; Polydisciplinaire de Taroudant depuis le <strong>{{hire_date}}</strong>."),
            P("La pr&eacute;sente attestation est d&eacute;livr&eacute;e &agrave; l'int&eacute;ress&eacute; sur sa demande pour servir et valoir ce que de droit."),
            '<div style="margin-top:50px; text-align:right;">'
            '<p>Fait &agrave; Taroudant le &nbsp; {{date_today}}</p>'
            '<p style="margin-top:50px; color:#888;">[Signature et cachet]</p>'
            '</div>'
        ),
    },
