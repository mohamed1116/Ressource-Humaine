...
class FreeRequestCreateView(generics.CreateAPIView):
    serializer_class = DocumentRequestCreateSerializer
    permission_classes = [IsAuthenticated]
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            hr_users = User.objects.filter(role__in=['ADMIN_HR', 'SUPER_ADMIN'])
...
class DocumentDownloadView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not gen_doc and doc_req.template and doc_req.status in (
            DocumentRequest.Status.APPROVED,
            DocumentRequest.Status.GENERATED,
            DocumentRequest.Status.PENDING_SIGNATURE,
        ):
