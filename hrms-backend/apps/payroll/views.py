...
class MarkAsPaidView(APIView):
    """POST /payroll/payslips/<uuid>/mark-paid/"""
    permission_classes = [IsAdminHR]

    def post(self, request, pk):
        try:
            payslip = Payslip.objects.get(pk=pk)
            PayrollService.mark_as_paid(payslip)
            return Response(PayslipSerializer(payslip).data)
        except Payslip.DoesNotExist:
            return Response({'detail': 'Not found.'}, status=status.HTTP_404_NOT_FOUND)
...
class AssignSalaryStructureView(APIView):
    """POST /payroll/employee-salaries/assign/"""
    permission_classes = [IsAdminHR]
    ...
    def post(self, request):
        ...
        base_override    = request.data.get('base_salary_override')
        effective_date   = request.data.get('effective_date')

        if not employee_id or not structure_id:
            return Response({'detail': 'employee_id et structure_id sont requis.'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            employee  = Employee.objects.get(pk=employee_id)
            structure = SalaryStructure.objects.get(pk=structure_id)
        except (Employee.DoesNotExist, SalaryStructure.DoesNotExist) as e:
            return Response({'detail': str(e)}, status=status.HTTP_404_NOT_FOUND)

        emp_salary = PayrollService.assign_salary_structure(
            employee, structure, base_override, effective_date
        )
        return Response(EmployeeSalarySerializer(emp_salary).data)
