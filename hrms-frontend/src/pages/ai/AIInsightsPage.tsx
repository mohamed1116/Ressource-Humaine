...
import { getLeavePredictions, getLatePatterns, getDepartmentAlerts, getRecommendations } from '../../api/ai.api';
...
export default function AIInsightsPage() {
  ...
  useEffect(() => {
    Promise.all([
      getLeavePredictions().then(res => setLeavePredictions(res.data)).catch(() => {}),
      getLatePatterns().then(res => setLatePatterns(res.data)).catch(() => {}),
      getDepartmentAlerts().then(res => setAlerts(res.data)).catch(() => {}),
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">AI Insights & Analytics</h1>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Leave Predictions */}
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold mb-4">Leave Forecast</h2>
            ...
            <div className="space-y-2">
              {(leavePredictions.forecast as Array<Record<string, unknown>>).slice(0, 6).map((f, i) => (
                <div key={i} className="flex justify-between items-center py-2 border-b">
                  ...
                  <span className="font-medium">{f.predicted_leaves as number} leaves</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500">Not enough historical data for predictions.</p>
          )}
        </div>

        {/* Late Arrival Patterns */}
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold mb-4">Late Arrival Patterns</h2>
              ...
              {(latePatterns.flagged_employees as Array<Record<string, unknown>>).slice(0, 5).map((emp, i) => (
                <div key={i} className="flex justify-between items-center py-2 border-b">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500 mt-1">Risk: {emp.risk_score as number}</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500">No concerning patterns detected.</p>
          )}
        </div>

        {/* Department Alerts */}
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold mb-4">Department Alerts</h2>
              ...
              {(recommendations.generated as Array<Record<string, unknown>>).map((rec, i) => (
                <div key={i} className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-500">Priority: {rec.priority as number}</span>
                  </div>
