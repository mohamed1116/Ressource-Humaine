                ...
                return (
                  <tr key={r.id as string} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-gray-700">
                      {r.template_name
                        ? r.template_name as string
                        : <span className="inline-block px-2 py-0.5 rounded text-xs font-medium border bg-amber-50 text-amber-700 border-amber-200">Demande libre</span>
                      }
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1.5 flex-wrap">
                        {/* Free request: show attachment link instead of preview/generate */}
                        {!r.has_template ? (
                          <>
                            <Btn onClick={() => handlePreview(r.id as string)} color="gray">Apercu</Btn>
                            {r.attachment && (
                              <a
                                href={`${(import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000/api/v1').replace('/api/v1', '')}${r.attachment}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="px-2.5 py-1 text-xs font-medium border rounded transition-colors text-amber-700 border-amber-200 hover:bg-amber-50"
                              >
                                📎 Pièce jointe
                              </a>
                            )}
